...
// Function to make API call to AIML (correct image generation endpoint)
async function callAIMLAPI(prompt, outputPath) {
  try {
    console.log(`🔄 Calling AIML API for: ${path.basename(outputPath)}`);
    
    // Use the correct AIML API endpoint for image generation
    ...
    if (!data.images || !data.images[0] || !data.images[0].url) {
        console.log('❌ No image URL in response. Full response:', JSON.stringify(data, null, 2));
        throw new Error('No image URL received from AIML API');
    }
